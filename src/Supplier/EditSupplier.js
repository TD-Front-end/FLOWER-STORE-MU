...
class EditSupplier extends React.Component {
    ...
    componentDidMount() {
        ...
        const getSupplier = async () => {
            try {
                const res = await axios.get(`http://localhost:4000/api/supplier/${this.props.supplierId}`)
                this.setState({
                    //item: res.data,
                    DataisLoaded: true,
    ...
    handleUpdate = event => {
        ...
        (async () => {
            ...
            await fetch(`http://localhost:4000/api/supplier/${this.props.supplierId}`, requestOptions);
        })();
        // .then(res => {
